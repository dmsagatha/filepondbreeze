    ...
    <div class="md:w-1/2 p-10 bg-white rounded-lg shadow-xl mt-10 md:mt-0">
      <form action="{{ route('articles.store') }}" method="POST" novalidate>
        ...
        <div class="mb-5">
          ...
          <x-text-input type="text" id="name" name="name" class="block border p-3 w-full rounded-lg @error('name') border-red-500 @enderror" :value="$article->name ?? old('name')" autofocus autocomplete="name" placeholder="Nombre del Artículo" />
          @error('name')
        ...
        <div class="mb-5">
          <input name="imagen" type="hidden" class="border p-3 w-full rounded-lg bg-red-50" value="{{ old('imagen') }}" />
          @error('imagen')
            <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
              {{ $message }}
            </p>
