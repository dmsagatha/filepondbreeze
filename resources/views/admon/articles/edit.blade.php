<x-app-layout>
  ...
  <div class="md:flex md:items-center py-2">
    ...
    <div class="md:w-1/2 p-10 bg-white rounded-lg shadow-xl mt-10 md:mt-0">
      <form action="{{ route('articles.update', $article) }}" method="POST" novalidate>
        @csrf @method('put')
        ...
        <div class="mb-5">
          ...
          <textarea id="description" name="description"
            class="border p-3 w-full rounded-lg  @error('description') border-red-500 @enderror">{{ $article->description ?? old('description') }}</textarea>
          @error('description')
            <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
              {{ $message }}
            </p>
