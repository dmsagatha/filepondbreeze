<x-app-layout>
    <x-slot:header>
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ isset($article) ? __('Edit') : __('Create') }}
        </h2>
    </x-slot:header>
                    ...
                    <form method="post" action="{{ route('articles.update', $article->id) }}" class="mt-6 space-y-6"
                        ...
                        <div>
                            ...
                            <x-text-input type="text" id="name" name="name" class="block mt-1 w-full"
                                :value="$article->name ?? old('name')" autofocus autocomplete="name" />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="col-md-8 mb-4">
                            <div class="dropzone" id="dropzone">
                                <img class="" src="{{ asset('storage/articles/' . $article->image) }}" class="img-thumbnail"
            ...
            let myDropzone = new Dropzone("#dropzone", {
                ...
                init: function() {
                    @if (isset($article) && $article->featured_image)
                        var files =
                            {!! json_encode($article->featured_image) !!}
                        for (var i in files) {
                            var file = files[i]
