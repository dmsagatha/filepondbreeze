<x-app-layout>
  ...
  <div class="md:flex md:items-center py-2">
    ...
    <div class="md:w-1/2 p-10 bg-white rounded-lg shadow-xl mt-10 md:mt-0">
      <form method="post" action="{{ isset($article) ? route('articles.update', $article->id) : route('articles.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        @csrf
        @isset($article)
          @method('put')
        @endisset
        ...
        <div class="mb-5">
          ...
          <textarea id="description" name="description" class="border p-3 w-full rounded-lg  @error('description') border-red-500 @enderror">{{ $article->description ?? old('description') }}</textarea>
          @error('description')
            <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
              {{ $message }}
            </p>
        ...
        <div class="mb-5">
          <input name="image" type="hidden" class="border-dashed border-2 border-indigo-600 p-3 w-full rounded-lg bg-red-50" value="{{ (isset($article)) ? $article->image : old('image') }}" />
          @error('image')
            <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
              {{ $message }}
            </p>
        ...
        <div class="flex items-center justify-center mt-4">
          <x-primary-button class="ml-4">
            {{ isset($article->id) ? __('Update') : __('Create') }}
          </x-primary-button>
